...
> The business domain is **repo-specific (🔴)** — capture *yours* from code/tickets. What's generic (🟢)
> is the *shape* legacy enterprise LOB apps tend to share. Don't assume specifics; verify.

## Common enterprise-LOB shapes (🟢 patterns, not facts about your repo)
...
## How to capture your domain (🔴 — fill from your repo)
- 🔴 Glossary: the 10–20 domain terms a newcomer hears week one (with the code symbol each maps to).
- 🔴 The 3–5 **hot modules** your tickets actually touch (start memory notes there).
- 🔴 Status/type **code tables** (single-char codes → meaning), cited to the enum/constant/query.
- 🔴 Any **geo/region forks** in business rules.
