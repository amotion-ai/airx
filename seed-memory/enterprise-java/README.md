...
> what your repo looks like; **your code proves it.** Every line is tagged — confirm [verify]/[fill] against code
> with a real `file:line`, then keep it. Nothing here is client-specific; it's the archetype's family traits.

**Legend:** [family] family (true of any repo in this archetype) / [verify] verify (likely true here — confirm) /
[fill] repo-specific (create/fill from your code).
...
2. Skim `MEMORY.md`, then run predict-and-verify on each `SEED-*.md`: confirm every [verify]/[fill] with `file:line`.
...
> The seed predicts; the target repo's code proves. Never present a [verify]/[fill] prediction as fact.
