...
- **9 verified `file:line` citations [family], 10 explicitly marked "verify" [verify].** Nothing fuzzy was stated as fact.
- **Predict-and-verify caught 2 would-be errors** before they shipped: a method whose name is *misspelled
  ...
  didn't match the first mapping — flagged [verify], not asserted. *A store/index tool would have embedded both.*
